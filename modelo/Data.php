...
//    
    ...
    public function listaEquipo() {
        ...
        while ($row = $res->fetch_array()) {
            ...
            echo" </button>";
            echo " <ul class = 'dropdown-menu' role = 'menu'>";
            echo " <li><a onclick = Limpiar('$row[0]')> Eliminar</a></li>";
    ...
    public function listaFunEqu() {
        ...
        while ($row = $res->fetch_array()) {
            ...
            if ($e == 1 && $el == 1) {
                ...
                echo " <li><a href='#'>Sin Privilegios</a></li>";
            }
            echo "</td>";
            echo" </tr>";
        }
...
//  
    public function comboCentroFA($nombre) {
        ...
        $res = $this->c->ejecutar($sql);
        echo "<select id='centro' name='centro' class='form-control' readonly>";
    ...
    public function comboCentroFiltrado($centro) {

        $sql = "select id,nombre from centro where id ='" . $centro . "'";
    ...
    public function comboFuncionarioF($nombre) {
        ...
        while ($resultado = $res->fetch_array()) {
            if ($nombre === $resultado[1]) {
                echo "<option value='" . $resultado[0] . "' selected> " . $resultado[1] . " " . $resultado[2] . "</option>";
            } else {
    ...
    public function comboIp() {
        ...
        $sql = "select ip from equipo WHERE nombre is null";

        $res = $this->c->ejecutar($sql);
        echo "<select id = 'txtIp' name='txtIp' class='form-control'>";
        while ($resultado = $res->fetch_array()) {

            echo "<option value='" . $resultado[0] . "'> IP: " . $resultado[0] . "</option>";
        }
        echo "</select>";
    }

    public function comboIpFiltradoEditar($ip) {

        $sql = "select ip from equipo WHERE ip ='.$ip.'";
    ...
    public function comboIpF($ip) {

        $sql = "select ip from equipo WHERE nombre <> 'null' and ip ='.$ip.'";
    ...
    public function comboIpFFF($ip, $ce) {

        $sql = "select ip from equipo WHERE nombre <> 'null' and ip ='.$ip.' and  ip and ip like '%." . $ce . ".%'";
    ...
    public function comboIpCentro($ce) {

        $sql = "select ip from equipo WHERE nombre <> 'null' and ip like '%." . $ce . ".%'";
...
//  
    ...
    public function upip($ip) {
        $sql = "UPDATE `agendaCormun`.`equipo` SET `nombre` = null, `box` = null, `sector` = null, `centro` = null, `anexo` = '0'  WHERE `equipo`.`ip` = '" . $ip . "'";
